...
s_list <- lapply(news_result_test$content, function(x) {
  tokens <- segment(x, jiebar = jieba_worker)
  tokens <- tokens[nchar(tokens) > 1]
  return(tokens)
  })
...
n_grams <- lapply(s_list, function(x){
  vapply(ngrams(x, 2), paste, "", collapse = " ")
  })
n_grams

#jieba函数
jieba_tokenizer_bigrams <- function(t) {
  jieba_worker <- worker(stop_word = "stopwordsCN.txt")  #设定分词引擎
  ...
  bigrams <- lapply(tokens, function(x){
    vapply(ngrams(x, 2), paste, "", collapse = " ")
    })
  return(bigrams)
}
news_result %>% unnest_tokens(input = content, output = word, token = jieba_tokenizer_bigrams)
```
